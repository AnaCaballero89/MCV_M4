function [u] = Ana_Laplace_Equation_Axb(f, dom2Inp,~)
%function [u] = Ana_Laplace_Equation_Axb(f, dom2Inp, param)
...
% ======================================
% Preallocating
idx_Ai = zeros((ni+2)*4+(nj+2)*4,1);
idx_Aj = zeros((ni+2)*4+(nj+2)*4,1);
a_ij = zeros((ni+2)*4+(nj+2)*4 +1,1);
...
% ======================================
    ...
    % sparse(i,j,v,m,n,nz) allocates space for nz nonzero elements. 
    % Use this syntax to allocate extra space for nonzero values to be filled in after construction.
    % i,j — Subscript pairs
    % v — values
    % m,n — Size of each dimension
    % nz — Storage allocation nonzero elements
