...
%% F. Matrix im1, im2
%  We compute fundamental matrix between every image and
%  the reference image (image_1)
matches_1_2 = siftmatch(desc_1, desc_2);
figure;
plotmatches(im1, im2, points_1(1:2,:), points_2(1:2,:), matches_1_2, 'Stacking', 'v');

% p1 and p2 contain the homogeneous coordinates of the matches
p1 = [points_1(1:2, matches_1_2(1,:)); ones(1, length(matches_1_2))];
p2 = [points_2(1:2, matches_1_2(2,:)); ones(1, length(matches_1_2))];

% ToDo: create this function (you can use as a basis 'ransac_homography_adaptive_loop.m')
[F1_2, inliers] = ransac_fundamental_matrix(p1, p2, 2.0); 

% show inliers
figure;
plotmatches(im1, im2, points_1(1:2,:), points_2(1:2,:), matches_1_2(:,inliers), 'Stacking', 'v');
title('Inliers');
vgg_gui_F(im1rgb, im2rgb, F1_2');

%% F. Matrix im1, im3
matches_1_3 = siftmatch(desc_1, desc_3);
figure;
plotmatches(im1, im3, points_1(1:2,:), points_3(1:2,:), matches_1_3, 'Stacking', 'v');

% p1 and p3 contain the homogeneous coordinates of the matches
p1 = [points_1(1:2, matches_1_3(1,:)); ones(1, length(matches_1_3))];
p3 = [points_3(1:2, matches_1_3(2,:)); ones(1, length(matches_1_3))];

% ToDo: create this function (you can use as a basis 'ransac_homography_adaptive_loop.m')
[F1_3, inliers] = ransac_fundamental_matrix(p1, p3, 2.0); 

% show inliers
figure;
plotmatches(im1, im3, points_1(1:2,:), points_3(1:2,:), matches_1_3(:,inliers), 'Stacking', 'v');
title('Inliers');
vgg_gui_F(im1rgb, im3rgb, F1_3');

%% F. Matrix im1, im4
matches_1_4 = siftmatch(desc_1, desc_4);
figure;
plotmatches(im1, im4, points_1(1:2,:), points_4(1:2,:), matches_1_4, 'Stacking', 'v');

% p1 and p4 contain the homogeneous coordinates of the matches
p1 = [points_1(1:2, matches_1_4(1,:)); ones(1, length(matches_1_4))];
p4 = [points_4(1:2, matches_1_4(2,:)); ones(1, length(matches_1_4))];
